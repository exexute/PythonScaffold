...
def ydl_all_ip():
	...
	a_s = soup.find(name = 'div', id ='MBC0').find_all(name = 'ul')[1].find_all(name = 'a')
	for a in a_s:
		proxy_spider(a['href'])
		#urls.append(a['href'])
...
def proxy_spider(url):
	...
	r = requests.get(url = url, headers = headers)
	soup = bs(r.text, "html.parser")
	ips = soup.find(name = 'div', attrs={'class':'content',}).find_all(name = 'p')
...
def save2db():
	...
	for proxy in free_proxy:
		#插入数据
		keys = list(proxy.keys())#在py2中，dict.keys()获得列表；py3中，获得dict_key()对象，需要使用list转换为列表后使用
		key = keys[0]
		sock = proxy[key]
		print(key, sock)
		sql.execute_with_param("INSERT INTO proxy_ip(sock, types) VALUES(%s, %s)", (sock, key))
